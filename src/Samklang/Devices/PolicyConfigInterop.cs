...
/// <summary>
/// P/Invoke-free COM interop for <c>IPolicyConfig</c> — the undocumented Windows Audio Policy
/// Config interface used to read and write a device's shared-mode Device Format. There is no
/// public Windows SDK header for this; the CLSID/IID pair and vtable layout below are the
/// interface Windows itself exposes (unavoidably identical across every tool that talks to it,
/// e.g. WindowsLosslessSwitcher, EarTrumpet, AudioSwitcher), reachable only via raw COM activation.
/// </summary>
internal static class PolicyConfigInterop
{
    ...
    private static IPolicyConfig CreatePolicyConfig()
    {
        var policyConfigType = Type.GetTypeFromCLSID(PolicyConfigClientClsid, throwOnError: true)
            ?? throw new InvalidOperationException("Unable to resolve the IPolicyConfig COM type.");
